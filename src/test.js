...
// ---------- 1. course, objectives, in class ----------
head('course, objectives, in class');
ok(/Kotler/.test(A.COURSE.about) && A.COURSE.chapters.length === 4, 'course description and four chapters');
tps.forEach((tp, i) => {
  ok(A.OBJECTIVES[tp].length >= 4 && A.OBJECTIVES[tp].every(o => new RegExp('^' + (i + 5) + '\\.\\d').test(o)), 'objectives numbered for ' + tp, A.OBJECTIVES[tp].length);
});
ok(A.OBJECTIVES.c6.length === 5 && /institutional and government/.test(A.OBJECTIVES.c6[4]), 'chapter 6 has five objectives (from the SmartArt)');
...
ok(A.GUIDE.sections.length === 4 && A.GUIDE.sections.every((s, i) => s.tp === tps[i]), 'four sections, one per chapter');
...
ok(items.filter(i => i.know).length >= 14, 'the know-it-cold items are marked', items.filter(i => i.know).length);
items.forEach(i => {
...
tps.forEach((tp, k) => {
  ...
  ok(c.notes.length >= 6 && c.notes.every(n => n.id && n.h && n.body && n.body.length > 300), 'notes complete: ' + tp, c.notes.length);
  c.notes.forEach(n => { ok(n.id.indexOf(tp + '-') === 0, 'note id prefixed: ' + n.id); noteIds.push(n.id); });
  ok(c.decks.length === 2 && c.decks.every(d => d.id && d.label && d.cards.length >= 15), 'two decks with 15+ cards: ' + tp);
  c.decks.forEach(d => {
...
ok(/\$234\.90/.test(body('c5')) && /\$0\.79/.test(body('c5')), 'Beyond Meat figures');
...
['$30 trillion', '34.8 million', '99.9%', '$16.48 billion', '$184 billion', '9" × 12"'].forEach(v => ok(body('c6').includes(v), 'ch. 6 class figure: ' + v));
// chapter 7
...
ok(/40%/.test(body('c7')) && /twice/.test(body('c7')) && /\$1\.75–2\.0 billion/.test(body('c7')), 'Tide and Starbucks figures');
...
['Line extensions', 'Brand extensions', 'Multibrands', 'New brands'].forEach(v => ok(body('c8').includes('<b>' + v + '</b>'), 'ch. 8 brand development: ' + v));
ok(/\$470\.9B/.test(body('c8')) && /\$1\.48T/.test(body('c8')) && /\$645/.test(body('c8')) && /\$4\.95 trillion/.test(body('c8')), 'ch. 8 class figures');
...
tps.forEach(tp => {
  ...
  ok(mine.length >= 50, 'at least 50 questions on ' + tp, mine.length);
  ok(mine.filter(q => q.t === 'tf').length >= 10, 'true/false on ' + tp, mine.filter(q => q.t === 'tf').length);
  ok(mine.filter(q => q.ap).length >= 6, 'application questions on ' + tp, mine.filter(q => q.ap).length);
});
A.QB.forEach((q, i) => {
...
['div', 'section', 'button', 'nav', 'main', 'header', 'footer', 'svg', 'symbol', 'table', 'g', 'ol', 'ul'].forEach(t => {
